...
import Boxing_Glove from "../assets/boxing-glove.png"; 
...
import Sweat from "../assets/heart-rate.png"; 
import TimeLength from "../assets/icons8-timer-96.png";
...
const weekDaysInitials = ['Su', 'M', 'Tu', 'W', 'Th', 'Fr', 'Sa'];
const [selectedWeekDay, setSelectedWeekDay] = useState(new Date().getDay());
const WeekDaysUI = ()=>{
    return(
        <XGroup columnGap={10} top = {0}>
            {
        weekDaysInitials.map((item, idx) =>{
                    return(
                        <TouchableOpacity key = {item} onPress={() => setSelectedWeekDay(idx)}>
                        <Avatar circular > {/*</TouchableOpacity>borderRadius = {0}  borderTopColor = {idx == selectedWeekDay? "rgba(0,0,0,.75)" : "rgba(0,0,0,.05)"} borderStyle="solid"  borderTopWidth={idx == selectedWeekDay?1 : 0} borderBottomWidth={idx == selectedWeekDay? 0 : 0} paddingTop={0} >*/}
                            <Text style = {{ paddingTop: 2.5, color: idx == selectedWeekDay? "black" : "rgba(0,0,0,.45)", borderTopColor:"black", borderTopWidth: idx == selectedWeekDay? 1.5 : 0, borderStyle:"solid", paddingHorizontal: 7, fontSize: 15, fontWeight: idx == selectedWeekDay? "400" : "300"}}>{item}</Text>
                        </Avatar>
                        </TouchableOpacity>
                    )
             })
...
const LastPerformanceStats = () => 
    <YGroup alignItems="center" justifyContent="center" marginTop={0} backgroundColor={"transparent"} >
            
        <XGroup justifyContent="space-between" alignItems="center" columnGap={27} marginVertical={2}>
            <XGroup.Item>
                 <TouchableOpacity style = {{rowGap: 7, borderRadius: 15, paddingHorizontal: 15, paddingVertical: 5}}>
        <Image source={Boxing_Glove} style = {{height: 30, width: 30, bottom: -3,alignSelf:"center"}}/>


<XGroup>
                <Text color = "lightgray" style = {{textAlign:"center",color: "rgba(0,0,0,.75)" }}>1.5/s</Text>
                
</XGroup>
            </TouchableOpacity>
            </XGroup.Item>
           
            <Separator vertical borderColor = "lightgray" borderWidth={.75} height = {80}/>

              <XGroup.Item>
                <TouchableOpacity style = {{rowGap: 7, borderRadius: 15, paddingHorizontal: 12, paddingVertical: 5}}>
                                <Image source={TimeLength} style = {{height: 30, width: 30, alignSelf:"center"}}/>

  
                 <Text color ="lightgray" style = {{textAlign:"center", color: "rgba(0,0,0,.75)" }}>3 mins</Text>
            </TouchableOpacity>
            </XGroup.Item>
      
            <Separator vertical borderColor={"lightgray"} borderWidth={.75} height={80}/>
              <XGroup.Item>
                <TouchableOpacity style = {{rowGap: 7, borderRadius: 15,  paddingVertical: 5}}>
                        <Image source={Sweat} style = {{height: 30, width: 30, alignSelf:"center"}}/>
                  <Text color = "lightgray" style = {{textAlign:"center", color: "rgba(0,0,0,.65)"}}>130 cals</Text>
            </TouchableOpacity>
            </XGroup.Item>
...
<Card.FeaturedTitle style = {{ paddingHorizontal: 15, paddingVertical: 15, color:"rgba(0,0,0,1)", textAlign:"center", fontWeight: 400, fontSize: 12}}> 
  Continnue your streak with the Jab!    
   The most important punch type in the book, dont miss out on it!
</Card.FeaturedTitle>
     ...
     <LastPerformanceStats/>
         ...
         }} style = {{marginTop: 20, marginBottom: 0, backgroundColor:"rgba(0,0,0,.85)", alignSelf:"center",  alignItems:"center", justifyContent:"center", borderRadius: 100, borderWidth: 0, paddingVertical: 5, paddingHorizontal: 25, width: ScreenWidth/1.25 }}>
   ...
   */
   ...
   /*
     <View style = {{bottom:-30}}>
        <Progress.Bar width={ScreenWidth*.7} collapsable progress = {.75} color="#FF8000" borderRadius={100} useNativeDriver = {true} animationType="spring" unfilledColor="white"/>
        </View>
        */
...
return(
  ...
  <View style = {[styles.row, {width:ScreenWidth, justifyContent:"left", alignItems:"left", alignContent:"left", backgroundColor:"white", height: ScreenHeight*.12, zIndex: 1000}]}>

        <SideNav buttonColor={theme? "black": "black"} style = {{ top: 70, left: 23}}/>
</View>

      <ScrollView horizontal = {false} style = {{top:7}} centerContent contentContainerStyle = {{alignItems:"center", justifyContent:"center",  }} showsVerticalScrollIndicator = {false} >

   
   <WeekDaysUI/>


<Card containerStyle = {{backgroundColor:"#2285FF", borderRadius:10, borerWidth: 0, outlineWidth: 0, width: ScreenWidth*.89, paddingBottom: 35,}}>
   <YGroup alignItems="center">
    <XGroup style = {{ marginVertical: 10, alignSelf:"center", justifyContent:"center", alignItems:"center", textAlign:"center", columnGap: 2}}>
        <Card.Image source = {Fire} borderRadius={10} height={75} width={75} style = {{height: 50, width: 50,}}/>
        <Text style = {{fontSize: 37, alignSelf:"center", bottom: -5, color:"white", fontFamily:"Inter"}}>12 </Text><Text style = {{fontSize: 14, top:-7, left: -10}}></Text>
    
    </XGroup>
            <Text style = {{fontSize: 15, alignSelf:"center", bottom: -7, color:"white", textAlign:"center", fontFamily:"Inter"}}>Almost half way to a month streak! Great job! </Text>
...
<View style = {{justifyContent:"center", backgroundColor:"transparent", paddingVertical: 10, paddingBottom: 37, marginTop: 10}}>
